...
int main() {
    ...
    // ==========================================
    // 1. REGISTER STREAMING MIME TYPES
    // ==========================================
    ...
    // It is also highly recommended to explicitly register your audio chunks 
    // so the browser's Web Audio API doesn't complain!
    ...
    svr.Post("/api/upload", [&](const httplib::Request& req, httplib::Response& res) {
        ...
        // 1. Read the song name from the Custom HTTP Header
        ...
        // 2. The file is literally the entire body of the request!
        if (req.body.empty()) {
            res.status = 400;
        ...
        // 3. Save the raw binary data directly to a temporary file
        std::string tempPath = "./temp_audio.tmp";
        std::ofstream ofs(tempPath, std::ios::binary);
        ofs << req.body;
        ofs.close();

        // 4. Create the new folder architecture
      ...
      std::cout << "[UPLOADER] Encoding 64k tier...\n";
        ...
        // 6. Generate the master.m3u8 file
